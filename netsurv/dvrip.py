...
class DVRIPCam(object):
    ...
    def build_packet(self, input_data, message_code, encoding="ascii"):
        ...
        # Number of packets sent in current session
        sequence_number = bytes(chr(self.packet_count), 'utf-8')
        unknown_block_1 = b"\x00\x00\x00\x00\x00"
        # message code from definition table, little-endian order
        message_byte = struct.pack('<H', message_code)

        # Size of data in bytes (padded to 4 bytes)
        data_len = struct.pack('<I', len(data) + 1)
        # ascii data, maximum of 16kb, terminated with a null ascii character
        data = bytes(data + "\x0a", 'utf-8')

        packet = head_flag + version + reserved_01 + reserved_02 + session_id + \
            unknown_block_0 + sequence_number + unknown_block_1 + message_byte + data_len + data
        return packet
